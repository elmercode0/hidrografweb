...
Navegação (menu lateral): página **Análise** (fluxo principal) e página **FAQ**.
"""
...
REPO_URL = "https://github.com/elmercode0/hidrografweb"
...
def page_analise() -> None:
    """Página principal: upload/exemplo na sidebar + abas de análise e diagramas."""
    # ------------------------------------------------------------------------ Sidebar
    ...
    use_example = st.sidebar.toggle("Usar dados de exemplo", value=False)
...
def page_faq() -> None:
    """Perguntas frequentes sobre uso, formatos, unidades e análises."""
    st.sidebar.title("💧 Hidrograf")
    st.sidebar.caption("Perguntas frequentes sobre o uso da aplicação.")
    st.sidebar.markdown(f"[📦 Código-fonte no GitHub]({REPO_URL})")

    st.title("❓ FAQ — Perguntas frequentes")
    st.caption("Dúvidas comuns sobre formatos de planilha, unidades e as análises disponíveis.")

    with st.expander("O que é o Hidrograf e o que ele faz?", expanded=True):
        st.markdown(
            "O **Hidrograf** é uma reimplementação em Python, de código aberto, dos módulos de "
            "análise hidroquímica do **QualiGraf** (FUNCEME). A partir de uma planilha de "
            "amostras de água ele calcula **balanço iônico**, **STD/CONAMA**, "
            "**classificação para irrigação (SAR/USSL)**, **IQA**, **estatísticas** e "
            "**correlações**, e gera os **diagramas** de Piper, Stiff, Durov, "
            "Schoeller-Berkaloff e Radial."
        )

    with st.expander("Como envio meus dados? Que formatos são aceitos?"):
        st.markdown(
            "Envie uma planilha **CSV** ou **XLSX** pela barra lateral (página **Análise**). "
            "Cada linha é uma amostra; as colunas são identificadas por apelido "
            "(ex.: `Na`, `Sodio`, `Na+` são reconhecidos como sódio).\n\n"
            "Não tem uma planilha? Use os botões **Modelo CSV** / **Modelo Excel** na barra "
            "lateral para baixar um arquivo pronto, preencher com seus dados e enviar. "
            "Para só experimentar, ative **Usar dados de exemplo**."
        )

    with st.expander("A planilha aceita mg/L e meq/L? Como diferencio?"):
        st.markdown(
            "Sim, aceita as duas unidades e converte tudo para **mg/L** internamente. "
            "Há duas formas de indicar a unidade:\n\n"
            "1. **Planilha inteira** — ao enviar, escolha no seletor "
            "*“Unidade das concentrações de íons”* (mg/L ou meq/L).\n"
            "2. **Por coluna** — anote a unidade no cabeçalho, ex.: `Na (meq/L)`, `Ca (epm)`, "
            "`Cl_mg/L`. Aceita `meq`/`epm` para meq/L e `mg/L`/`(mg)`/`mg` para mg/L.\n\n"
            "O cabeçalho por coluna **tem precedência** sobre o seletor, então dá para "
            "misturar unidades. Obs.: `Mg` sozinho é sempre lido como o íon **magnésio**, "
            "nunca como “miligrama”."
        )

    with st.expander("Quais colunas/parâmetros o Hidrograf entende?"):
        st.markdown(
            "Íons principais: **Ca, Mg, Na, K** (cátions) e **Cl, SO₄, HCO₃, CO₃** (ânions). "
            "Parâmetros físico-químicos: **CE** (µS/cm), **pH**, **TDS**. Para o **IQA** são "
            "necessários os 9 parâmetros do índice (ex.: OD/`do_sat`, coliformes, DBO, "
            "N total, P total, turbidez, sólidos totais, variação de temperatura, pH). "
            "Colunas ausentes apenas desabilitam as análises que dependem delas."
        )

    with st.expander("Os resultados do IQA são oficiais?"):
        st.markdown(
            "Trate como **estimativa**. As curvas qi foram digitalizadas da Figura 01 do "
            "QualiGraf e corroboradas pelas equações da CETESB (diferença de ±2–3 q). "
            "O IQA exige os 9 parâmetros; sem todos eles, o índice não é calculado."
        )

    with st.expander("Meus dados ficam salvos em algum lugar?"):
        st.markdown(
            "Não. O processamento acontece **em memória**, a cada sessão; nada é persistido "
            "no servidor. Ao fechar a aba, os dados enviados somem."
        )

    with st.expander("Como cito ou credito o trabalho original?"):
        st.markdown(
            "O Hidrograf é inspirado no **QualiGraf**, de **Gilberto Möbus** (FUNCEME/CE) — "
            "[qualigraf.funceme.br](https://qualigraf.funceme.br/). Esta é uma reimplementação "
            "independente em Python; os créditos do software original são da "
            "[FUNCEME](http://www.funceme.br/)."
        )

    st.markdown(f"**Mais detalhes e código:** [{REPO_URL}]({REPO_URL})")
    _footer()


nav = st.navigation([
    st.Page(page_analise, title="Análise", icon="💧", default=True),
    st.Page(page_faq, title="FAQ", icon="❓"),
])
nav.run()
